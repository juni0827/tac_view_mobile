...
interface OperationsPanelProps {
  ...
  onMapTilesChange: (tile: MapTilesMode) => void;
  osmTerrainEnabled: boolean;
  onOsmTerrainToggle: () => void;
  osmBuildingsEnabled: boolean;
  onOsmBuildingsToggle: () => void;
  showPaths: boolean;
...
export default function OperationsPanel({
  ...
  const panelContent = (
    <>
      ...
      {mapTiles === 'osm' && (
        <div className="p-3 border-b border-wv-border">
          <div className="text-[9px] text-wv-muted tracking-widest uppercase mb-2">OSM 3D</div>
          <div className="flex flex-col gap-1">
            <button
              onClick={onOsmTerrainToggle}
              className={`
            ...
            >
              ...
              <span className="tracking-wider">TERRAIN 3D</span>
              <span className={`ml-auto w-1.5 h-1.5 rounded-full ${osmTerrainEnabled ? 'bg-wv-cyan' : 'bg-wv-muted/30'}`} />
            </button>
            <button
              onClick={onOsmBuildingsToggle}
              className={`
                flex items-center gap-2 px-2 py-1.5 rounded text-[10px] w-full
                transition-all duration-200 text-left
                ${isMobile ? 'min-h-[44px]' : ''}
            ...
            >
              ...
              <span className="tracking-wider">BUILDINGS 3D</span>
              <span className={`ml-auto w-1.5 h-1.5 rounded-full ${osmBuildingsEnabled ? 'bg-wv-green' : 'bg-wv-muted/30'}`} />
            </button>
          </div>
        </div>
      )}
