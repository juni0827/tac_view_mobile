...
function getRelatedEndpoint(
  relation: OntologyEntityDetail['relations'][number],
  selectedEntityId: string,
) {
  return relation.sourceEntityId === selectedEntityId ? relation.target : relation.source;
}
        ...
        <div className="grid grid-cols-2 gap-2 text-[10px]">
          <label className="col-span-2 flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <span className="text-wv-muted">AOI Radius</span>
              <span className="text-[9px] uppercase tracking-[0.2em] text-wv-cyan">{filters.radiusKm} km</span>
            </div>
            <input
              type="range"
              min={MIN_RADIUS_KM}
              ...
              onChange={(event) => onFiltersChange({ ...filters, radiusKm: Number(event.target.value) || MIN_RADIUS_KM })}
              className="w-full accent-cyan-400"
            />
            <div className="flex items-center justify-between text-[9px] uppercase tracking-[0.18em] text-wv-muted">
              <span>{MIN_RADIUS_KM} km</span>
              <span>circle around current camera</span>
            ...
            <div className="rounded border border-wv-border bg-black/20 p-3">
              ...
              <div className="space-y-1">
                {selectedEntity.relations.slice(0, 8).map((relation) => {
                  const relatedEndpoint = getRelatedEndpoint(relation, selectedEntity.id);
                  return (
                    <button
                      key={relation.id}
                      type="button"
                      onClick={() => onSelectEntity(relatedEndpoint.id)}
                      className="flex w-full items-center justify-between gap-2 rounded border border-transparent px-2 py-1 text-left text-[10px] text-wv-text transition-colors hover:border-wv-border/70 hover:bg-white/5"
                    >
                      <span className="min-w-0 flex-1">
                        <span className="block truncate">{relatedEndpoint.label}</span>
                        <span className="block text-[9px] uppercase text-wv-muted">
                          {relatedEndpoint.canonicalType.replace(/_/g, ' ')}
                        </span>
                      </span>
                      <span className="shrink-0 text-wv-amber">{relation.relationType.replace(/_/g, ' ')}</span>
                    </button>
                  );
                })}
