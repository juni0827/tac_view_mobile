...
interface IntelFeedItem {
  id: string;
  time: string;
  type: 'flight' | 'seismic' | 'satellite' | 'system' | 'cctv' | 'ship' | 'osint' | 'conflict';
  message: string;
  href?: string;
}
  ...
  const [bootMessages] = useState<IntelFeedItem[]>([
    {
      ...
      type: 'system',
      message: 'TAC_VIEW v1.0.0 INITIALISING...',
    },
    {
      id: 'boot-2',
      time: new Date().toISOString().slice(11, 19),
        ...
        <div key={item.id} className={`flex gap-2 py-0.5 text-[9px] leading-tight ${isMobile ? 'py-1.5 text-[11px]' : ''}`}>
          ...
          {item.href ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-wv-text/80 underline decoration-wv-border/60 underline-offset-2"
            >
              {item.message}
            </a>
          ) : (
