...
interface GlobeViewerProps {
  ...
  mapTiles: MapTilesMode;
  osmTerrainEnabled: boolean;
...
const GOOGLE_MAPS_CREDIT = new Credit('Google Maps');
...
export default function GlobeViewer({
  ...
  const osmTerrainEnabledRef = useRef(osmTerrainEnabled);
  const osmBuildingsEnabledRef = useRef(osmBuildingsEnabled);
  ...
  const osmTerrainRequestIdRef = useRef(0);
  const osmBuildingsRequestIdRef = useRef(0);
  const googlePhotoRequestIdRef = useRef(0);
  ...
  useEffect(() => {
    ...
    osmTerrainEnabledRef.current = osmTerrainEnabled;
    osmBuildingsEnabledRef.current = osmBuildingsEnabled;
  }, [mapTiles, onCameraChange, onCameraMoveEnd, onMapNotice, onRenderFailure, onViewerReady, osmBuildingsEnabled, osmTerrainEnabled]);

  const clearMapNotice = useCallback(() => {
    onMapNoticeRef.current?.(null);
  }, []);

  const setMapNotice = useCallback((message: string) => {
    onMapNoticeRef.current?.(message);
  }, []);
  ...
  const applyGoogleSatelliteImagery = useCallback(async (viewer: CesiumViewer) => {
    if (googleApiKey) {
      GoogleMaps.defaultApiKey = googleApiKey;
    }

    const provider = await Google2DImageryProvider.fromUrl({
      key: googleApiKey,
      mapType: 'satellite',
      credit: GOOGLE_MAPS_CREDIT,
    });
    ...
    viewer.imageryLayers.removeAll();
    viewer.imageryLayers.addImageryProvider(provider as never);
  }, [googleApiKey]);

  const applyFlatTerrain = useCallback((viewer: CesiumViewer) => {
    if (viewer.isDestroyed()) {
      return;
    }
  ...
  const removeOsmBuildings = useCallback((viewer: CesiumViewer) => {
    if (viewer.isDestroyed() || !osmBuildingsTilesetRef.current) {
      return;
    }
  ...
  const applyOsmTerrain = useCallback(async (viewer: CesiumViewer) => {
    if (!osmTerrainEnabledRef.current) {
    ...
    const requestId = ++osmTerrainRequestIdRef.current;
    ...
    try {
      ...
      if (
        viewer.isDestroyed() ||
        requestId !== osmTerrainRequestIdRef.current ||
        mapTilesRef.current !== 'osm' ||
        !osmTerrainEnabledRef.current
      ) {
        return;
      }
      ...
      console.warn('[TAC_VIEW] OSM terrain failed to load.', error);
      applyFlatTerrain(viewer);
      setMapNotice(getOsm3dFailureNotice('terrain', error, Boolean(cesiumIonToken)));
    }
  }, [applyFlatTerrain, cesiumIonToken, clearMapNotice, setMapNotice]);

  const applyGooglePhotoRelief = useCallback(async (viewer: CesiumViewer) => {
    if (cesiumIonToken) {
      Ion.defaultAccessToken = cesiumIonToken;
    }

    const requestId = ++googlePhotoRequestIdRef.current;

    try {
      await applyGoogleSatelliteImagery(viewer);
      ...
      if (cesiumIonToken) {
        ...
        if (
          viewer.isDestroyed() ||
          requestId !== googlePhotoRequestIdRef.current ||
          mapTilesRef.current !== 'google'
        ) {
          return;
        }
      ...
      console.warn('[TAC_VIEW] Google satellite relief failed to load.', error);
      applyOSM(viewer);
      applyFlatTerrain(viewer);
      setMapNotice(getGooglePhotoFailureNotice(error, Boolean(cesiumIonToken)));
    }
  }, [applyFlatTerrain, applyGoogleSatelliteImagery, cesiumIonToken, setMapNotice]);

  const applyOsmBuildings = useCallback(async (viewer: CesiumViewer) => {
    const requestId = ++osmBuildingsRequestIdRef.current;

    if (!osmBuildingsEnabledRef.current) {
    ...
    try {
      const tileset = await createOsmBuildingsAsync({
        defaultColor: Color.fromCssColorString('#d7dde5').withAlpha(0.92),
      ...
      if (
        viewer.isDestroyed() ||
        requestId !== osmBuildingsRequestIdRef.current ||
        mapTilesRef.current !== 'osm' ||
        !osmBuildingsEnabledRef.current
      ) {
        return;
      }
      ...
      console.warn('[TAC_VIEW] OSM buildings failed to load.', error);
      removeOsmBuildings(viewer);
      setMapNotice(getOsm3dFailureNotice('buildings', error, Boolean(cesiumIonToken)));
    }
  }, [cesiumIonToken, clearMapNotice, removeOsmBuildings, setMapNotice]);
  ...
  const handleViewerReady = useCallback(async (viewer: CesiumViewer) => {
    ...
    if (mapTiles === 'google' && googleApiKey) {
      try {
        ...
        console.warn('Google 3D Tiles failed to load, falling back to Google satellite relief.', error);
        if (globe) {
          globe.show = true;
        }
        await applyGooglePhotoRelief(viewer);
        removeOsmBuildings(viewer);
        setMapNotice(getGoogle3dFailureNotice(error));
      }
      ...
      applyFlatTerrain(viewer);
      removeOsmBuildings(viewer);
      setMapNotice(getMissingGoogle3dNotice());
    } else {
      if (globe) {
        globe.show = true;
      }
      applyOSM(viewer);
      clearMapNotice();
      await applyOsmTerrain(viewer);
      await applyOsmBuildings(viewer);
    }
  ...
  }, [applyFlatTerrain, applyGooglePhotoRelief, applyOsmBuildings, applyOsmTerrain, clearMapNotice, emitCamera, googleApiKey, loadGoogleTiles, mapTiles, removeOsmBuildings, setMapNotice]);
    ...
    if (mapTiles === 'google' && !google3dReadyRef.current && googleApiKey) {
      ...
      void (async () => {
        try {
          ...
          console.warn('Google 3D Tiles failed, staying on Google satellite relief.', error);
          if (globe) {
            globe.show = true;
          }
          await applyGooglePhotoRelief(viewer);
      ...
      applyOSM(viewer);
      clearMapNotice();
      void applyOsmTerrain(viewer);
      void applyOsmBuildings(viewer);
    } else if (mapTiles === 'google') {
      if (globe) {
  ...
  }, [applyFlatTerrain, applyGooglePhotoRelief, applyOsmBuildings, applyOsmTerrain, clearMapNotice, googleApiKey, loadGoogleTiles, mapTiles, removeOsmBuildings, setMapNotice]);
  ...
  return (
    ...
    >
      ...
      <Globe
        enableLighting={mapTiles === 'google' || (mapTiles === 'osm' && osmTerrainEnabled)}
