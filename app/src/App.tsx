...
import { useIntelBriefing } from './intel/useIntelBriefing';
...
import type { MapTilesMode, RenderCameraState } from './types/rendering';
...
import { getDefaultMapTiles } from './lib/basemaps';
...
function getInitialMapTiles(): MapTilesMode {
  try {
    return getDefaultMapTiles(getRuntimeClientConfig().googleApiKey);
  } catch {
    return 'osm';
  }
}
...
function App() {
  ...
  const [mapTiles, setMapTiles] = useState<MapTilesMode>(() => getInitialMapTiles());
  const [osmTerrainEnabled, setOsmTerrainEnabled] = useState(true);
  const [osmBuildingsEnabled, setOsmBuildingsEnabled] = useState(true);
  ...
  const handleRenderFailure = useCallback((error: unknown) => {
    ...
    setShaderMode('none');
    setMapTiles('osm');
    setOsmTerrainEnabled(false);
    setOsmBuildingsEnabled(false);
  ...
  const { ships: rawShips, feedItems: shipFeedItems, isLoading: shipsLoading } = useShips(layers.ships, false);
  const {
  ...
  const { feedItems: intelBriefingFeedItems } = useIntelBriefing(queryCamera);
  ...
  const allFeedItems: IntelFeedItem[] = useMemo(
    () => [
      ...
      ...cctvFeedItems,
      ...shipFeedItems,
      ...intelBriefingFeedItems,
      ...visualIntelligence.feedItems,
    ],
    [cctvFeedItems, eqFeedItems, fltFeedItems, intelBriefingFeedItems, satFeedItems, shipFeedItems, visualIntelligence.feedItems],
  );
  ...
  return (
    <div data-testid="app-root" className={`w-screen h-screen bg-wv-black overflow-hidden ${opticsOverlayEnabled ? 'scanline-overlay' : ''}`}>
      ...
      <GlobeViewer
        ...
        mapTiles={mapTiles}
        osmTerrainEnabled={osmTerrainEnabled}
        osmBuildingsEnabled={osmBuildingsEnabled}
      ...
      {!renderRecoveryNotice && mapNotice && (
        <div className="absolute top-4 left-1/2 z-50 -translate-x-1/2 rounded border border-amber-500/60 bg-black/85 px-4 py-2 text-xs tracking-[0.16em] text-amber-200">
          {mapNotice}
        </div>
      )}
      ...
      <OperationsPanel
        ...
        onOsmTerrainToggle={() => {
          audio.play('click');
          setMapNotice(null);
          setOsmTerrainEnabled((value) => !value);
        }}
        osmBuildingsEnabled={osmBuildingsEnabled}
        onOsmBuildingsToggle={() => {
          audio.play('click');
          setMapNotice(null);
          setOsmBuildingsEnabled((value) => !value);
        }}
        showPaths={showPaths}
