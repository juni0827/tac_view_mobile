 ...
 *
 * Loaded by the packaged desktop sidecar bootstrap.
 */
...
import { fetchOverpassElementsByTiles } from './overpass.js';
...
import { registerIntelRoutes } from './intel/routes.js';
import { createIntelService, getExternalIntelRuntimeStatus } from './intel/service.js';
...
const intelService = createIntelService({ getCachedValue, rememberValue });
...
app.get('/api/traffic/roads', async (req, res) => {
  try {
    ...
    let roads = null;
    try {
      const overpassResult = await fetchFromOverpass(bbox);
      roads = overpassResult.roads;
      if (roads && roads.length > 0) {
        ...
        return res.json({
          roads,
          cached: false,
          source: overpassResult.failedTileCount > 0 ? 'overpass-partial' : 'overpass',
        });
      }
    } catch (overpassErr) {
...
async function fetchFromOverpass(bbox) {
  const { elements, failedTiles } = await fetchOverpassElementsByTiles({
    bbox,
    buildQuery: (tile) => `
[out:json][timeout:12];
...
out tags geom qt;
`,
    urls: runtimeConfig.server.ontologyOverpassUrls,
    timeoutMs: 12000,
    maxTileSpan: 0.5,
    minTileSpan: 0.2,
    abortAfterTerminalFailures: 4,
    onRequestError: (_url, error) => {
      if (error.message.includes('HTTP 429')) {
        console.warn(`[TRAFFIC] ${error.message}, trying next server...`);
        return;
      }
  ...
  if (failedTiles.length > 0) {
    console.warn(`[TRAFFIC] Completed with ${failedTiles.length} failed tile(s)`);
  }
...
registerIntelRoutes(app, intelService);
...
app.get('/api/health', (_req, res) => {
  res.json({
    ...
    runtime: {
      ...
      externalIntel: getExternalIntelRuntimeStatus(runtimeConfig),
    },
  });
});
...
// Regional bounding boxes for global coverage (10 zones 횞 up to 1500 each ??full global)
const FR24_ZONES = [
...
if (isDirectRun) {
  server.listen(PORT, () => {
    console.log(`
?붴븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧????  TAC_VIEW PROXY SERVER               ????  Port: ${PORT}                          ????  WebSocket: ws://localhost:${PORT}/ws    ???싢븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧??    `);
  });
}
