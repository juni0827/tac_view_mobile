...
import {
  ...
  fetchOverpassElementsByTiles,
  normalizeOverpassUrls,
} from '../overpass.js';
...
const GENERAL_FACILITY_AMENITY_PATTERN = '^(hospital|clinic|police|fire_station|college|university|school|prison|courthouse|townhall|embassy)$';
const GENERAL_FACILITY_BUILDING_PATTERN = '^(hospital|civic|government|public)$';
const GENERAL_FACILITY_MAN_MADE_PATTERN = '^(works|wastewater_plant|water_works)$';
...
function resolveBetterSqliteNativeBinding() {
  ...
  const candidateDirs = [
    ...
    path.join(repoRoot, 'app', 'desktop', 'binaries'),
...
function inferInfrastructureSubtype(tags) {
  return tags.military
    || tags.power
    || tags.highway
    || tags.railway
    || tags.amenity
...
function normalizeOsmElement(element, fetchedAt, enrichment = null) {
  ...
  return {
    ...
    canonicalType,
    subtype: inferInfrastructureSubtype(tags),
    label: label || refId,
    aliases,
    origin: 'observed',
...
function buildOverpassQuery(bbox, entityTypes) {
  ...
  if (wants.has('facility')) {
    parts.push(`node["amenity"~"${GENERAL_FACILITY_AMENITY_PATTERN}"]${bounds};`);
    parts.push(`way["amenity"~"${GENERAL_FACILITY_AMENITY_PATTERN}"]${bounds};`);
    parts.push(`node["office"="government"]${bounds};`);
    parts.push(`way["office"="government"]${bounds};`);
    parts.push(`node["building"~"${GENERAL_FACILITY_BUILDING_PATTERN}"]${bounds};`);
    parts.push(`way["building"~"${GENERAL_FACILITY_BUILDING_PATTERN}"]${bounds};`);
    parts.push(`node["man_made"~"${GENERAL_FACILITY_MAN_MADE_PATTERN}"]${bounds};`);
    parts.push(`way["man_made"~"${GENERAL_FACILITY_MAN_MADE_PATTERN}"]${bounds};`);
  }
...
export function createOntologyService({ dbPath, snapshotStore = null, connectorConfig = {} }) {
  ...
  const overpassUrls = normalizeOverpassUrls(ensureStringArray(connectorConfig.overpassUrls, DEFAULT_OVERPASS_URLS));
  ...
  function beginRun(connectorName, scopeKey = 'global') {
    const id = `run-${connectorName}-${randomUUID()}`;
  ...
  function finishRun(id, connectorName, scopeKey, status, recordCount, error = null, metadata = {}) {
    ...
    const isSuccessful = status === 'success' || status === 'partial';
    finishRunStmt.run({
      id,
      finishedAt,
      status,
    ...
    connectorStatusStmt.run({
      ...
      lastSuccessAt: isSuccessful ? finishedAt : null,
      lastError: error,
  ...
  const rebuildRelationsTx = db.transaction((entityIds) => {
    ...
    const facilities = allEntities.filter((entity) => entity.canonicalType === 'facility');
    const roads = allEntities.filter((entity) => entity.canonicalType === 'road_segment');
    ...
    for (const entity of allEntities) {
      ...
      if (entity.canonicalType === 'sensor') {
        for (const candidate of allEntities) {
          if (!['aircraft', 'vessel', 'earthquake', 'facility'].includes(candidate.canonicalType)) {
            continue;
          }

          const limitKm = candidate.canonicalType === 'aircraft'
            ? 250
            : candidate.canonicalType === 'vessel'
              ? 180
              : candidate.canonicalType === 'facility'
                ? 40
                : 120;
          const distanceKm = haversineKm(
            entity.geometry.latitude,
          ...
          if (distanceKm <= limitKm) {
            createRelation({
              relationType: candidate.canonicalType === 'facility' ? 'facility_monitoring' : 'sensor_overlap',
              sourceEntityId: entity.id,
              targetEntityId: candidate.id,
              confidence: clamp(1 - distanceKm / limitKm, 0.35, 0.88),
              ruleName: candidate.canonicalType === 'facility' ? 'sensor_near_facility' : 'sensor_proximity',
              metadata: { distanceKm },
              symmetric: candidate.canonicalType === 'facility',
              description: candidate.canonicalType === 'facility'
                ? `${entity.label} is positioned to monitor ${candidate.label}.`
                : `${entity.label} is within observational range of ${candidate.label}.`,
            });
          }
        }
      }

      if (entity.canonicalType === 'facility') {
        for (const sensor of sensors) {
          const distanceKm = haversineKm(
            entity.geometry.latitude,
            entity.geometry.longitude,
          ...
          if (distanceKm <= 40) {
            createRelation({
              relationType: 'facility_monitoring',
              sourceEntityId: entity.id,
              targetEntityId: sensor.id,
              confidence: clamp(1 - distanceKm / 40, 0.35, 0.88),
              ruleName: 'facility_near_sensor',
              metadata: { distanceKm },
              symmetric: true,
              description: `${entity.label} is near ${sensor.label}.`,
            });
          }
        }

        for (const candidate of [...roads, ...bridgeLike, ...airports, ...ports]) {
          const limitKm = candidate.canonicalType === 'road_segment'
            ? 3
            : candidate.canonicalType === 'airport' || candidate.canonicalType === 'port'
              ? 35
              : 6;
          const distanceKm = haversineKm(
            entity.geometry.latitude,
          ...
          if (distanceKm <= limitKm) {
            createRelation({
              ...
              sourceEntityId: entity.id,
              targetEntityId: candidate.id,
              confidence: clamp(1 - distanceKm / limitKm, 0.4, 0.86),
              ruleName: candidate.canonicalType === 'road_segment'
                ? 'facility_near_road'
                : candidate.canonicalType === 'airport' || candidate.canonicalType === 'port'
                  ? 'facility_near_transport_hub'
                  : 'facility_near_infrastructure',
              metadata: { distanceKm },
              symmetric: true,
  ...
  async function fetchOverpassInfrastructure(bbox, entityTypes, scopeKey) {
    if (!buildOverpassQuery(bbox, entityTypes)) {
      return {
        elements: [],
        failedTiles: [],
        snapshotUsed: false,
    ...
    try {
      const result = await fetchOverpassElementsByTiles({
        bbox,
        buildQuery: (tile) => buildOverpassQuery(tile, entityTypes),
        urls: overpassUrls,
        timeoutMs: 15000,
        maxTileSpan: 0.9,
        minTileSpan: 0.3,
        abortAfterTerminalFailures: 4,
        onRequestError: (url, error) => {
          console.warn(`[ONTOLOGY] Overpass tile request failed via ${url}: ${error.message}`);
        },
      });

      if (snapshotStore && result.elements.length > 0) {
        await snapshotStore.write(snapshotKey, {
          elements: result.elements,
          failedTileCount: result.failedTiles.length,
        });
      }
  ...
  async function ensureInfrastructureForSearch({ bbox, layerIds }) {
    ...
    try {
      const infrastructureResult = await fetchOverpassInfrastructure(bbox, entityTypes, scopeKey);
      const elements = infrastructureResult.elements;
      ...
      const status = infrastructureResult.failedTiles.length > 0 || infrastructureResult.snapshotUsed
        ? 'partial'
        : 'success';
      finishRun(runId, 'osm_infrastructure', scopeKey, status, records.length, null, {
        bbox,
        entityTypes,
        failedTileCount: infrastructureResult.failedTiles.length,
        snapshotUsed: infrastructureResult.snapshotUsed,
      });
      if (infrastructureResult.failedTiles.length > 0) {
        console.warn(`[ONTOLOGY] Infrastructure refresh completed with ${infrastructureResult.failedTiles.length} failed tile(s)`);
      }
    } catch (error) {
