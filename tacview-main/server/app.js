...
app.use('/api', (req, res, next) => {
  ...
  const providedToken = req.get('x-tac-view-token');
  if (providedToken === runtimeConfig.authToken) {
    next();
    return;
  }
...
// ?Ä?Ä?Ä Cache ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
const cache = new NodeCache();

async function getCachedValue(cacheKey, snapshotTtlSeconds = 5) {
  const cached = cache.get(cacheKey);
  if (cached !== undefined) {
    return cached;
  }

  const snapshot = await snapshotStore.read(cacheKey);
  if (snapshot !== undefined) {
    cache.set(cacheKey, snapshot, snapshotTtlSeconds);
    return snapshot;
  }
...
function rememberValue(cacheKey, value, ttlSeconds) {
  cache.set(cacheKey, value, ttlSeconds);
  void snapshotStore.write(cacheKey, value);
  return value;
}

// ?Ä?Ä?Ä OpenSky OAuth2 Token Management ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
// ?Ä?Ä?Ä REST Endpoints ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä

/** GET /api/earthquakes */
app.get('/api/earthquakes', async (_req, res) => {
  try {
    const cached = await getCachedValue('earthquakes');
    ...
    rememberValue('earthquakes', data, 60);
    res.json(data);
  } catch (err) {
  ...
  try {
    ...
    try {
      ...
      const celestrakRes = await fetch(
        ...
        {
          headers: {
            'User-Agent': 'TAC_VIEW-Satellite-Tracker/1.0 (desktop)',
            'Accept': 'text/plain',
          },
        }
      );
    ...
    rememberValue(cacheKey, tleText, 7200); // Cache 2 hours
  ...
  try {
    ...
    try {
      ...
      if (roads && roads.length > 0) {
        ...
        rememberValue(cacheKey, roads, 86400);
    ...
    if (bboxOverlapsSydney(bbox.south, bbox.west, bbox.north, bbox.east)) {
      ...
      rememberValue(cacheKey, filtered, 86400);
...
// ?Ä?Ä?Ä CCTV Camera Endpoints ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
app.get('/api/cctv', async (req, res) => {
  try {
    ...
    const responseCacheKey = `cctv-response:${country || 'all'}:${sourceFilter || 'all'}`;
    const cachedResponse = await getCachedValue(responseCacheKey);
    ...
    const results = await Promise.allSettled(
      activeSources.map(async (s) => {
        const cached = await getCachedValue(s.key);
        if (cached !== undefined) return cached;

        const data = await s.fetch();
        rememberValue(s.key, data, s.ttl);
    ...
    rememberValue(responseCacheKey, payload, 300);
    res.json(payload);
  } catch (err) {
...
// ?Ä?Ä?Ä Ship / AIS Tracking (AISStream.io) ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
app.get('/api/ships', async (req, res) => {
  try {
    ...
    let allVessels = await getCachedValue('ships-all');
    if (allVessels === undefined) {
      ...
      rememberValue('ships-all', allVessels, 60);
    }
    ...
    rememberValue('ships-moving', movingVessels, 60);
...
// Regional bounding boxes for global coverage (10 zones √ó up to 1500 each ??full global)
const FR24_ZONES = [
...
async function fetchFR24Zone(bounds) {
  ...
  const res = await fetch(url, {
    headers: {
      'User-Agent': 'Mozilla/5.0 (compatible; TAC_VIEW/1.0)',
...
function parseFR24Aircraft(id, arr) {
  ...
  // arr[0] is the real ICAO24 transponder hex; `id` is FR24's internal key ??  // only use the transponder hex so keys match adsb.fi for dedup
...
app.get('/api/flights', async (_req, res) => {
  try {
    ...
    rememberValue(cacheKey, flights, 30); // Cache 30 seconds (FR24 is now background enrichment)
    res.json(flights);
  } catch (err) {
...
app.get('/api/flights/live', async (req, res) => {
  try {
    ...
    const adsbRes = await fetch(adsbUrl, {
      headers: {
        'User-Agent': 'TAC_VIEW-Tracker/1.0 (desktop)',
        'Accept': 'application/json',
      },
    });
    ...
    const routeMap = (await getCachedValue('fr24-route-registry')) || {};
    ...
    rememberValue(cacheKey, flights, 4); // Cache 4 seconds
    res.json(flights);
  } catch (err) {
...
async function refreshRouteRegistry() {
  try {
    ...
    rememberValue('fr24-route-registry', registry, 90); // valid for 90s
...
const backgroundTasksEnabled = process.env.TAC_VIEW_DISABLE_BACKGROUND_TASKS !== '1';
let routeRegistryInterval = null;
let routeRegistryTimeout = null;

if (backgroundTasksEnabled) {
  routeRegistryInterval = setInterval(refreshRouteRegistry, 60_000);
  routeRegistryTimeout = setTimeout(refreshRouteRegistry, 5_000);
}

// ?Ä?Ä?Ä WebSocket for real-time flight push ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
// ?Ä?Ä?Ä Export for Vercel Serverless ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
export function stopBackgroundTasks() {
  if (routeRegistryInterval) {
    clearInterval(routeRegistryInterval);
    routeRegistryInterval = null;
  }
...
export { app, runtimeConfig, server };
export default app;

// ?Ä?Ä?Ä Start (standalone mode only) ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
// This path is mainly used for local direct execution.
const isDirectRun = false;

if (isDirectRun) {
  server.listen(PORT, () => {
    console.log(`
?î‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê????  TAC_VIEW PROXY SERVER               ????  Port: ${PORT}                          ????  WebSocket: ws://localhost:${PORT}/ws    ???ö‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê??    `);
  });
}
