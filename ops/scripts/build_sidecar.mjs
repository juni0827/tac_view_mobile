...
const tempDir = path.join(rootDir, '.build-cache', 'sidecar');
const bundlePath = path.join(tempDir, 'tac_view-sidecar.cjs');
const binariesDir = path.join(rootDir, 'app', 'desktop', 'binaries');
...
async function main() {
  ...
  await build({
    entryPoints: [path.join(rootDir, 'app', 'server', 'bootstrap.js')],
    bundle: true,
    format: 'cjs',
    outfile: bundlePath,
    platform: 'node',
