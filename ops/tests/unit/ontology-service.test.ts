...
import { createOntologyService } from '../../../app/server/ontology/service.js';
...
describe('ontology service', () => {
  ...
  it('creates distinct ingestion runs even when syncs start at the same timestamp', async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2026-03-10T00:00:00.000Z'));

    try {
      await expect(service.syncFromFrontend({
        ...emptyPayload(),
        flights: [makeFlight({ icao24: 'abc123' })],
      })).resolves.toMatchObject({ ok: true });

      await expect(service.syncFromFrontend({
        ...emptyPayload(),
        flights: [makeFlight({ icao24: 'def456', callsign: 'TAC202' })],
      })).resolves.toMatchObject({ ok: true });

      const runCount = service.db.prepare(`
        SELECT COUNT(*) AS count
        FROM ingestion_runs
        WHERE connector_name = 'frontend_sync'
      `).get() as { count: number };
