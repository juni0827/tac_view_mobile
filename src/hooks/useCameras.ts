...
export function useCameras(enabled: boolean, countryFilter: string = 'ALL') {
  ...
  const fetchCameras = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/cctv?${params}`);
      ...
      const newFeed: IntelFeedItem[] = [
        {
          ...
          time,
          type: 'cctv' as IntelFeedItem['type'],
          message: `${data.meta.onlineCameras} cameras online across ${data.meta.countries.length} regions`,
        },
      ];
      setFeedItems(newFeed);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Unknown CCTV error';
      console.error('[CCTV] Fetch error:', message);
      setError(message);
      setRetryCount((c) => c + 1);
    } finally {
      setIsLoading(false);
